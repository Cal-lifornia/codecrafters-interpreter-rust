...
pub fn parse_exprs(ctx: &mut Context, min_bp: u8) -> Result<Expr, InterpreterError> {
    let first = ctx.statement().next_token();
    let mut lhs = match first {
        Token::LeftParen => {
            let lhs = Expr::Group(Box::new(parse_exprs(ctx, 0)?));
        ...
        _ => {
            if let Some(literal) = Literal::from_token(&first) {
                ...
                Expr::Unary(op, Box::new(parse_exprs(ctx, 7)?))
            } else {
                return Err(InterpreterError::Syntax(format!("invalid token: {first}")));
            }
        }
